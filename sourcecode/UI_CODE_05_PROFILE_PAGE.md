...
This page has two tabs: **Profile** (personal info + profile picture + barangay logo for officers) and **Security** (change password).
...
**Note:** This is the complete profile page with:
- **Profile Tab**: Profile picture upload (base64), first/last name editing, email (read-only), role display, and barangay logo upload (officers only)
- **Security Tab**: Change password with real-time validation (8+ chars, uppercase, lowercase, number, special char)
- All uploads validate file size (max 5MB) and file type (images only)
