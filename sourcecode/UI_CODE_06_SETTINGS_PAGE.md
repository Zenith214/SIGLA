...
**Admin Settings Panel** - Main admin control center with sidebar navigation

**Key Features:**
- **Collapsible sidebar** - Toggle with Menu/X button
- **Dynamic section loading** - Lazy-loaded sections for performance (SurveyCycles, Barangays, AwardManagement, SurveyTargets, UsersRoles, SupervisorAssignments, Backup)
- **Permission-based access** - Redirects non-admin users, backup-only users see only backup section
- **Sticky header** - Dark slate-800 header with breadcrumbs, live clock, active cycle display
- **Section routing** - Switch between 8 admin sections via sidebar
- **Access denied UI** - 🔒 lock icon with message for unauthorized sections
- **Loading skeleton** - Shows during initial 600ms page load

**Sections Available:**
1. Survey Cycles - Manage survey periods
2. Barangays - Barangay management
3. Award Management - Award configuration
4. Survey Targets - Target setting
5. Supervisor Assignments - FS assignments
6. Users & Roles - User management
7. Dashboard Settings - Dashboard config
8. Backup - Database backup (accessible to backup role)

**Header Elements:**
- Sidebar toggle button
- Breadcrumb navigation (PULSE Admin > Section Name)
- Live date/time clock (updates every second)
- Active cycle indicator (or "⚠️ No Active Cycle" warning)
