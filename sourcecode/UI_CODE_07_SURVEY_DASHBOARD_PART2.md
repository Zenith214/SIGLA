# SURVEY DASHBOARD (Field Interviewer Dashboard) - PART 2

**File**: `src/app/survey/page.tsx` (continued)
...
**Survey Dashboard** - Field interviewer main workspace with 3 tabs

**Key Features:**
- **Offline support** - OfflineIndicator + AutoSync components
- **3-tab interface** - Overall Progress, My Spots (for FI/admin/dev), Legacy Assignments (FI only)
- **Philippine time clock** - UTC+8 conversion, updates every second
- **Active cycle display** - Shows current survey cycle or warning
- **User dropdown menu** - Profile, Settings (admin only), Logout
- **Auto-refresh** - Refetches data on page visibility/focus change
- **Role-based UI** - Different views for interviewer vs admin/officer

**Tab 1: Overall Progress**
- 4 stat cards: Total targets, With assignments, Completed, In Progress
- Overall progress bar (calculated from all barangays)
- Grid of all barangay cards (4 columns on desktop)
- Each card shows: name, status badge, interviewer, progress bar, population/households
- Clickable cards link to `/survey/barangay/{id}`

**Tab 2: My Spots** (interviewer/admin/dev only)
- Shows `MySpotAssignments` component
- Spot-based assignment system (new approach)

**Tab 3: Legacy Assignments** (interviewer only)
- Shows only user's assigned barangays
- Action buttons: "Start Interview" or "Continue Interview" or "Survey Completed"
- Links to `/survey/forms?barangayId={id}`

**Header Elements:**
- PULSE logo
- Philippine time (hidden on mobile)
- Active cycle display (hidden on mobile)
- "Back to Dashboard" button (hidden for interviewers)
- User menu with profile picture/avatar

**Status Badges:**
- Completed (100%) - green
- In Progress (>0%) - blue
- Assigned (0% with assignment) - orange
- Not Assigned - gray
...
**Data Fetching:**
- Fetches from `/api/barangays-with-assignments`
- Cache-busting with timestamp parameter
- Filters user assignments for interviewers
- Auto-refresh on visibility change and window focus
